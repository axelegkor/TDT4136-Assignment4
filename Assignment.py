...
class CSP:
    ...
    # Function that checks if the value is consistent with the assignment

    def consistent(self, assignment, variable, value):
        neighbours = []
        neighbourArcs = self.get_all_neighboring_arcs(variable)
        for arc in neighbourArcs:
            if arc[0] == variable:
                neighbours.append(arc[1])
            else:
                neighbours.append(arc[0])
        for neighbour in neighbours:
            if len(assignment[neighbour]) == 1 and assignment[neighbour][0] == value:
                return False
        return True
    ...
    def backtrack(self, assignment):
        ...
        for value in assignment[variable]:
            ...
            if self.consistent(new_assignment, variable, value):
                new_assignment[variable] = [value]
                inferences = self.inference(new_assignment, None)
                if inferences != False:
...
print("Number of backtrack-calls:", csp.backtrackCounter)
print("Number of failures:", csp.failureCounter)
